...
def main(args):
    ...
    accelerator = Accelerator(
        ...
        project_dir=logging_dir, #logging_dir
    )
    ...
    print(f'len train dataloader : {len(train_dataloader)}')
    ...
    num_update_steps_per_epoch = math.ceil(len(train_dataloader) / args.gradient_accumulation_steps)
    print(f'num_update_steps_per_epoch2 : {num_update_steps_per_epoch}')
                    ...
                    if not args.not_cache_latents:
                        if args.train_text_encoder:
                            ...
                            if step % len(train_dataloader) == 0 :
                                #print(f'epoch: {epoch}')
                                #print(f'batch: {batch[0][1]}')
                                #print(f'hidden state size: {encoder_hidden_states.size()}')
                                #token_texts = [tokenizer.convert_ids_to_tokens(token_id) for token_id in batch[0][1]]
                                #print(f'token: {token_texts}')
                                import pandas as pd
                                import numpy as np
                                
                                z.append(encoder_hidden_states[0][4].cpu().detach().numpy().tolist()) # z
                                wx.append(encoder_hidden_states[0][5].cpu().detach().numpy().tolist()) # wx
                                style.append(encoder_hidden_states[0][6].cpu().detach().numpy().tolist()) # style
    ...
    df_z.insert(0,"batch",range(1,len(df_z)+1))
    df_wx.insert(0,"batch",range(1,len(df_z)+1))
    df_style.insert(0,"batch",range(1,len(df_z)+1))
    
    csv_filename_z = "./text_encoder_state/z.csv"
    csv_filename_wx = "./text_encoder_state/wx.csv"
    csv_filename_style = "./text_encoder_state/style.csv"
    
    df_z.to_csv(csv_filename_z, index=False)
    df_wx.to_csv(csv_filename_wx, index=False)
    df_style.to_csv(csv_filename_style, index=False)
